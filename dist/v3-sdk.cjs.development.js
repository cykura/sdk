...
var TickMath = /*#__PURE__*/function () {
  ...
  TickMath.getTickAtSqrtRatio = function getTickAtSqrtRatio(sqrtRatioX32) {
    !(JSBI.greaterThanOrEqual(sqrtRatioX32, TickMath.MIN_SQRT_RATIO) && JSBI.lessThan(sqrtRatioX32, TickMath.MAX_SQRT_RATIO)) ?  invariant(false, 'SQRT_RATIO')  : void 0; // we are not shifting in CYS
...
NoTickDataProvider.ERROR_MESSAGE = 'No tick data provider was given'; // export class SolanaTickDataProvider implements TickDataProvider {
...
var Pool = /*#__PURE__*/function () {
  ...
  function () {
    var _getOutputAmount = /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/runtime_1.mark(function _callee(inputAmount, sqrtPriceLimitX32) {
      var zeroForOne, _yield$this$swap, outputAmount, sqrtRatioX32, liquidity, tickCurrent, accounts, outputToken;
        ...
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 4:
              ...
              accounts = _yield$this$swap.accounts;
              // console.log('got tick from swap', tickCurrent)
              outputToken = zeroForOne ? this.token1 : this.token0;
              return _context.abrupt("return", [sdkCore.CurrencyAmount.fromRawAmount(outputToken, JSBI.multiply(outputAmount, NEGATIVE_ONE)), new Pool(this.token0, this.token1, this.fee, sqrtRatioX32, liquidity, tickCurrent, this.tickDataProvider), accounts]);
  ...
  function () {
    var _swap = /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/runtime_1.mark(function _callee3(zeroForOne, amountSpecified, sqrtPriceLimitX32) {
      var exactInput, state, lastSavedWordPos, step, nextInitTick, wordPos, bitmapAddress, _SwapMath$computeSwap, liquidityNet;
        ...
        while (1) {
          switch (_context3.prev = _context3.next) {
            ...
            case 10:
              ...
              wordPos = nextInitTick[2];
              bitmapAddress = nextInitTick[4]; // console.log('got next tick', step.tickNext)
  ...
  _createClass(Pool, [{
    ...
    get: function get() {
      ...
      if (this.token0.decimals !== this.token1.decimals) {
        if (this.token0.decimals < this.token1.decimals) {
          var p = new sdkCore.Price(this.token0, this.token1, Q64, JSBI.multiply(ratioX64, decimalMul));
          return p;
        } else {
          var _p = new sdkCore.Price(this.token0, this.token1, Q64, JSBI.divide(ratioX64, decimalMul));
        ...
        return (_this$_token0Price = this._token0Price) != null ? _this$_token0Price : this._token0Price = new sdkCore.Price(this.token0, this.token1, Q64, ratioX64);
      }
    }
    /**
    ...
    get: function get() {
      ...
      if (this.token0.decimals !== this.token1.decimals) {
        if (this.token0.decimals < this.token1.decimals) {
          var p = new sdkCore.Price(this.token0, this.token1, JSBI.divide(ratioX64, decimalMul), Q64);
          return p;
        } else {
          var _p2 = new sdkCore.Price(this.token0, this.token1, JSBI.multiply(ratioX64, decimalMul), Q64);
        ...
        return (_this$_token1Price = this._token1Price) != null ? _this$_token1Price : this._token1Price = new sdkCore.Price(this.token1, this.token0, ratioX64, Q64);
      }
    }
  }, {
    key: "chainId",
...
function tickToPrice(baseToken, quoteToken, tick) {
  ...
  if (baseToken.decimals !== quoteToken.decimals) {
    if (baseToken.decimals < quoteToken.decimals) {
      ...
      return new sdkCore.Price(baseToken, quoteToken, Q64, JSBI.multiply(ratioX64, decimalMul));
    } else {
      // WSOL USDC. USDC WSOL (invert)
      return new sdkCore.Price(baseToken, quoteToken, Q64, JSBI.divide(ratioX64, decimalMul));
    }
  } else {
  ...
  _createClass(Position, [{
    ...
    get: function get() {
      if (this._token0Amount === null) {
        if (this.pool.tickCurrent < this.tickLower) {
          this._token0Amount = sdkCore.CurrencyAmount.fromRawAmount(this.pool.token0, SqrtPriceMath.getAmount0Delta(TickMath.getSqrtRatioAtTick(this.tickLower), TickMath.getSqrtRatioAtTick(this.tickUpper), this.liquidity, false)); // console.log('AM0 less than tick Lowwer', this._token0Amount.toSignificant())
        } else if (this.pool.tickCurrent < this.tickUpper) {
          if (this.pool.token0.decimals < this.pool.token1.decimals) {
            this._token0Amount = sdkCore.CurrencyAmount.fromRawAmount(this.pool.token0, SqrtPriceMath.getAmount0Delta(this.pool.sqrtRatioX32, TickMath.getSqrtRatioAtTick(this.tickUpper), this.liquidity, false)); // console.log('AMO LT TU token0 < token1', this._token0Amount.toSignificant())
          } else {
            this._token0Amount = sdkCore.CurrencyAmount.fromRawAmount(this.pool.token0, // WSOL USDC and USDC WSOL all four ways? This works for both WSOL USDC, USDC WSOL entered both way for deposit amount inputs.
          ...
          this._token0Amount = sdkCore.CurrencyAmount.fromRawAmount(this.pool.token0, ZERO); // console.log('AM0 greater than tick upper', this._token0Amount.toSignificant())
        }
      }
    ...
    get: function get() {
      if (this._token1Amount === null) {
        if (this.pool.tickCurrent < this.tickLower) {
          this._token1Amount = sdkCore.CurrencyAmount.fromRawAmount(this.pool.token1, ZERO); // console.log('AM1 less than tick lower', this._token1Amount.toSignificant())
        } else if (this.pool.tickCurrent < this.tickUpper) {
          if (this.pool.token0.decimals < this.pool.token1.decimals) {
            this._token1Amount = sdkCore.CurrencyAmount.fromRawAmount(this.pool.token1, SqrtPriceMath.getAmount1Delta(TickMath.getSqrtRatioAtTick(this.tickLower), this.pool.sqrtRatioX32, this.liquidity, false)); // console.log('AM1 LT TU token0 < token1', this._token1Amount.toSignificant())
          } else {
            this._token1Amount = sdkCore.CurrencyAmount.fromRawAmount(this.pool.token1, SqrtPriceMath.getAmount1Delta(TickMath.getSqrtRatioAtTick(this.tickLower), this.pool.sqrtRatioX32, this.liquidity, false)); // console.log('AM1 LT TU token0 > token1', this._token1Amount.toSignificant())
          }
        } else {
          this._token1Amount = sdkCore.CurrencyAmount.fromRawAmount(this.pool.token1, SqrtPriceMath.getAmount1Delta(TickMath.getSqrtRatioAtTick(this.tickLower), TickMath.getSqrtRatioAtTick(this.tickUpper), this.liquidity, false)); // console.log('AM1 greater than Lowwer', this._token1Amount.toSignificant())
        }
      }
